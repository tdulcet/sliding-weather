#include "num2words.h"

#include <string.h>
#include <stdio.h>

static const char *const ONES[] = {
	"zero",
	"one",
	"two",
	"three",
	"four",
	"five",
	"six",
	"seven",
	"eight",
	"nine"
};

static const char *const TEENS[] = {
	"",
	"eleven",
	"twelve",
	"thirteen",
	"fourteen",
	"fifteen",
	"sixteen",
	"seventeen",
	"eighteen",
	"nineteen"
};

static const char *const TEENS_SPLIT[][2] = {
	{"", ""},
	{"eleven", ""},
	{"twelve", ""},
	{"thirteen", ""},
	{"four", "teen"},
	{"fifteen", ""},
	{"sixteen", ""},
	{"seven", "teen"},
	{"eight", "teen"},
	{"nine", "teen"}
};

static const char *const TENS[] = {
	"",
	"ten",
	"twenty",
	"thirty",
	"forty",
	"fifty",
	"sixty",
	"seventy",
	"eighty",
	"ninety"
};

// o'clock (0) and plain number words (10..)
void minute_to_formal_words(int minutes, char *first_word, char *second_word)
{
	// PBL_ASSERT(minutes >= 0 && minutes < 60, "Invalid number of minutes");
	first_word[0] = '\0';
	second_word[0] = '\0';

	if (minutes == 0)
	{
		strcat(first_word, "o'clock");
		return;
	}
	if (minutes < 10)
	{
		strcat(first_word, ONES[minutes]);
		return;
	}
	if (minutes > 10 && minutes < 20)
	{
		strcat(first_word, TEENS_SPLIT[minutes - 10][0]);
		strcat(second_word, TEENS_SPLIT[minutes - 10][1]);
		return;
	}

	strcat(first_word, TENS[minutes / 10]);

	const int minute_ones = minutes % 10;
	if (minute_ones)
	{
		strcat(second_word, ONES[minute_ones]);
	}
}

void hour_to_12h_word(int hours, char *word)
{
	// PBL_ASSERT(hours >= 0 && hours < 24, "Invalid number of hours");
	hours %= 12;

	if (hours == 0)
	{
		hours = 12;
	}

	word[0] = '\0';

	const int tens_val = hours / 10;
	const int ones_val = hours % 10;

	if (tens_val > 0)
	{
		if (tens_val == 1 && hours != 10)
		{
			strcat(word, TEENS[ones_val]);
			return;
		}
		strcat(word, TENS[tens_val]);
		if (ones_val > 0)
		{
			strcat(word, " ");
		}
	}

	if (ones_val > 0 || hours == 0)
	{
		strcat(word, ONES[ones_val]);
	}
}

void hour_to_24h_word(int hours, char *first_word, char *second_word)
{
	// PBL_ASSERT(hours >= 0 && hours < 24, "Invalid number of hours");
	// hours %= 24;

	first_word[0] = '\0';
	second_word[0] = '\0';

	if (hours < 10)
	{
		strcat(first_word, ONES[hours]);
		return;
	}
	if (hours > 10 && hours < 20)
	{
		strcat(first_word, TEENS_SPLIT[hours - 10][0]);
		strcat(second_word, TEENS_SPLIT[hours - 10][1]);
		return;
	}

	strcat(first_word, TENS[hours / 10]);

	const int hour_ones = hours % 10;
	if (hour_ones)
	{
		strcat(second_word, ONES[hour_ones]);
	}
}

void temperature_to_words(int temperature, char *first_word, char *second_word)
{
	int temp = temperature < 0 ? -temperature : temperature;

	first_word[0] = '\0';
	second_word[0] = '\0';

	if (temperature < 0)
	{
		strcat(first_word, "negative ");
	}

	if (temp < 10)
	{
		strcat(first_word, ONES[temp]);
		return;
	}

	if (temp >= 100)
	{
		strcat(first_word, ONES[temp / 100]);
		strcat(first_word, " hundred");
		temp %= 100;
		if (temp && temperature > 0)
		{
			strcat(first_word, " ");
		}
	}
	if (temp >= 20 || temp == 10)
	{
		strcat(temperature >= -100 ? first_word : second_word, TENS[temp / 10]);
		temp %= 10;
		if (temp && temperature <= 120)
		{
			strcat(temperature >= -100 ? first_word : second_word, " ");
		}
	}
	if (temp < 20 && temp > 10)
	{
		strcat(temperature >= -100 ? first_word : second_word, TEENS[temp - 10]);
	}
	else if (temp < 10 && temp > 0)
	{
		strcat(temperature <= 120 && temperature >= -100 ? first_word : second_word, ONES[temp]);
	}
}
